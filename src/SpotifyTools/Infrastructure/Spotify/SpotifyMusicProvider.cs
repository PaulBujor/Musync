...
public sealed class SpotifyMusicProvider(HttpClient http) : IMusicProvider
{
    ...
    public async IAsyncEnumerable<Track> GetSavedTracksAsync([EnumeratorCancellation] CancellationToken ct)
    {
        ...
        while (url is not null)
        {
            ...
            foreach (var item in page.Items)
                yield return new Track(item.Track.Id, item.Track.Name, item.Track.Artists.Count > 0 ? item.Track.Artists[0].Name : "Unknown Artist", item.Track.Album.Name, Isrc: null);
